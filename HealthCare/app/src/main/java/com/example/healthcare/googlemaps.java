...
public class googlemaps extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_pharmaci=findViewById(R.id.btn_nearest_pharmaci);
        btn_pharmaci.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        btn_hospital = findViewById(R.id.btn_nearest_hospital);
            ...
            public void onClick(View view) {
                ...
                locationProviderClient.getLastLocation().addOnSuccessListener(location -> {
                    if (location != null) {
                            ...
                            try {
                                ...
                                if (results.length() > 0) {
                                    ...
                                    for (int i = 0; i < results.length(); i++) {
                                        ...
                                        mMap.addMarker(new MarkerOptions().position(hospitalLatLng).title(hospital.getString("name")));
                                    }
                                    mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(location.getLatitude(), location.getLongitude()), 13));
                                } else {
                        ...
                        }, error -> Toast.makeText(getApplicationContext(), "Error finding hospitals", Toast.LENGTH_SHORT).show());
                        RequestQueue requestQueue = Volley.newRequestQueue(getApplicationContext());
                        requestQueue.add(jsonObjectRequest);
                    } else {
                        Toast.makeText(getApplicationContext(), "Unable to get current location", Toast.LENGTH_SHORT).show();
...
/*public class MainActivity extends AppCompatActivity implements OnMapReadyCallback {
    private GoogleMap mMap;
    private Button btnNearestHospital;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnNearestHospital = findViewById(R.id.btn_nearest_hospital);
        btnNearestHospital.setOnClickListener(view -> findNearestHospital());
    ...
    private void findNearestHospital() {
        ...
        PlacesApi.initialize(getApplicationContext(), apiKey);
        PlacesClient placesClient = PlacesApi.createClient(this);
        FusedLocationProviderClient locationProviderClient = LocationServices.getFusedLocationProviderClient(this);
        ...
        locationProviderClient.getLastLocation().addOnSuccessListener(location -> {
            if (location != null) {
                ...
                String requestUrl = String.format("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=%s&radius=%s&type=%s&key=%s", locationString, radius, placeType, apiKey);
                ...
                JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, requestUrl, null, response -> {
                    try {
                        JSONArray results = response.getJSONArray("results");
                        JSONObject nearestHospital = results.getJSONObject(0);
                        JSONObject geometry = nearestHospital.getJSONObject("geometry");
                        JSONObject locationObject = geometry.getJSONObject("location");
                        LatLng hospitalLatLng = new LatLng(locationObject.getDouble("lat"), locationObject.getDouble("lng"));
                        mMap.addMarker(new MarkerOptions().position(hospitalLatLng).title("Nearest Hospital"));
                        mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(hospitalLatLng, 15));
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                }, error -> Toast.makeText(this, "Error finding nearest hospital", Toast.LENGTH_SHORT).show());
